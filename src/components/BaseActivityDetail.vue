<template>
  <div class="activity-detail">
    <div v-if="!activity && !loading" class="not-found">
      <el-result
        icon="warning"
        title="活动不存在"
    ...
    <div v-else class="activity-content" v-loading="loading">
      ...
      <div class="activity-hero" v-if="activity">
        ...
        <div class="hero-content" v-if="activity">
          <h1 class="activity-title">{{ activity?.title || '活动标题' }}</h1>
          ...
          <div class="activity-meta">
            ...
            <div class="meta-info">
              ...
              <div class="info-row">
                ...
                <span>{{ activity?.participants || 0 }}/{{ activity?.maxParticipants || 0 }}人参与</span>
              </div>
            </div>
          </div>
          ...
          <div class="participation-section" v-if="activity?.status !== 'completed'">
            <el-progress
              v-if="(activity?.maxParticipants || 0) > 0"
              :percentage="((activity?.participants || 0) / (activity?.maxParticipants || 1)) * 100"
              :stroke-width="8"
              class="progress-large"
            />
            <div class="participation-info">
              <span>{{ (activity?.maxParticipants || 0) - (activity?.participants || 0) }} 个名额剩余</span>
              <el-button
                type="primary"
                size="large"
                :disabled="isDisabled"
      ...
      <el-row :gutter="30" class="content-section" v-if="activity">
            ...
            <div class="organizer-info">
              <div class="organizer-header">
                <el-avatar :size="50" :src="activity?.organizer?.avatar">
                  {{ activity?.organizer?.name?.charAt(0) || 'H' }}
                </el-avatar>
              ...
              <div class="contact-info" v-if="activity?.contact">
                <div class="contact-item">
                  ...
                  <span>{{ activity.contact?.phone || '暂无电话' }}</span>
                </div>
                <div class="contact-item">
                  <el-icon><Message /></el-icon>
                  <span>{{ activity.contact?.email || '暂无邮箱' }}</span>
                </div>
              </div>
            </div>
          </el-card>
...
const isDisabled = computed(() => {
  ...
  return props.activity?.status !== 'published' ||
         (props.activity?.participants || 0) >= (props.activity?.maxParticipants || 0) ||
...
const getButtonText = () => {
  // 管理员显示特定提示
  if (isAdmin.value) return '管理员无法报名'
  ...
  if ((props.activity?.participants || 0) >= (props.activity?.maxParticipants || 0)) return '名额已满'
  return '立即报名'
}
