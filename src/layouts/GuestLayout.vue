...
const userAvatarSrc = computed(() => {
  if (!authStore.user) return '/images/default-avatar.png'

  // 使用格式化函数处理头像URL
  const formattedAvatar = formatAvatarUrl(authStore.user.avatar)

  // 如果没有头像，使用默认头像
  return formattedAvatar || '/images/default-avatar.png'
})
...
const handleAvatarError = (event) => {
  // 如果用户头像加载失败，使用默认头像（基于用户名生成）
  const username = authStore.user?.username || authStore.user?.name
  if (username) {
    event.target.src = generateAvatarFromUsername(username)
  } else {
