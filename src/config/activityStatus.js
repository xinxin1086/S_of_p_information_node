...
export function getStatusText(status) {
  return STATUS_TRANSLATIONS[status] || '未知状态'
}
...
export function getStatusClass(status) {
  return STATUS_CLASS_MAP[status] || 'status-draft'
}

/**
 * 根据筛选条件获取对应的状态列表
 * @param {string} filter - 筛选条件：all, upcoming, ongoing, completed
 * @returns {string[]} 符合条件的状态数组
 */
export function getStatusesByFilter(filter) {
  switch (filter) {
    case 'all':
 ...
 * 包含草稿状态用于管理
 * @returns {Array<{label: string, value: string}>} 状态选项数组
 */
 ...
 * @returns {Array<{label: string, value: string}>} 状态选项数组
 */
export function getPublicStatusOptions() {
