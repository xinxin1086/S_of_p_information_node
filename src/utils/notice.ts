...
export const fetchNoticeList = async (
  ...
  try {
    ...
    const response = await api.get('/api/visit/notice', params)
...
export const fetchNoticeDetail = async (noticeId: number | string): Promise<NoticeDetail> => {
  try {
    const response = await api.get(`/api/visit/notice/${noticeId}`)
...
export const createAdminNotice = async (noticeData: Partial<NoticeDetail>) => {
  try {
    const requestData = {
      table_name: 'notice',
      operate_type: 'create',
      // 创建操作不需要查询条件，只需要要插入的数据
      kwargs: noticeData
    }

    console.log('🔄 创建公告请求数据:', requestData)
    const response = await api.post('/api/admin/operate', requestData)
...
export const updateAdminNotice = async (noticeId: number | string, noticeData: Partial<NoticeDetail>) => {
  try {
    // 根据后端API逻辑，更新公告应该使用 operate_type: 'edit'
    // 'edit' 操作直接使用 id 作为主键，kwargs 作为更新内容
    const requestData = {
      table_name: 'notice',
      operate_type: 'edit',
      id: noticeId,  // 直接使用 id 字段作为主键
      kwargs: noticeData
    }

    console.log('🔄 更新公告请求数据:', requestData)
    const response = await api.post('/api/admin/operate', requestData)
...
export const deleteAdminNotice = async (noticeId: number | string) => {
  try {
    ...
    console.log('🔄 删除公告请求数据:', requestData)
    const response = await api.post('/api/admin/operate', requestData)
