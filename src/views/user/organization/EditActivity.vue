...
const loadActivityData = async () => {
  try {
    loading.value = true
    const result = await activityStore.fetchActivity(activityId)
...
const submitForm = async (status) => {
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(status === 'draft' ? '草稿保存成功' : '活动更新成功')
...
const handleCancel = async () => {
  try {
    await ElMessageBox.confirm('确定要取消编辑活动吗？未保存的修改将丢失', '提示', {
